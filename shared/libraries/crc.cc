...
uint8_t get_crc8(void *ptr, uint16_t length) {
  ...
  uint8_t crc8 = k_crc8; // initial crc8
...
uint16_t get_crc16(void *ptr, uint16_t length) {
  uint8_t index;
  uint8_t *data = (uint8_t*)ptr;
  uint16_t crc16 = k_crc16; // initial crc16
...
bool check_crc8(void *ptr, uint16_t length) {
  ...
  return (get_crc8(ptr, length - 1) == data[length - 1]);
}

bool check_crc16(void *ptr, uint16_t length) {
  uint8_t *data = (uint8_t*)ptr;
  uint16_t crc16 = *(uint16_t*)(data + length - SIZEOF_CRC16);

  return (get_crc16(ptr, length - 2) == crc16);
}

void append_crc8(void *ptr, uint16_t length) {
  uint8_t *data = (uint8_t*)ptr;

  data[length] = get_crc8(ptr, length);
}

void append_crc16(void *ptr, uint16_t length) {
  uint8_t *data = (uint8_t*)ptr;
  uint16_t *crc16 = (uint16_t*)(data + length);

  *crc16 = get_crc16(ptr, length);
}
